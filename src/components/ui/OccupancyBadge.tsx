...
const OccupancyBadge: React.FC<OccupancyBadgeProps> = ({
  ...
  <div
    className={`flex flex-col gap-3 items-start rounded-2xl border border-gray-100 shadow-sm py-4 px-5 ${bgClass}`}
  >
    <span className={`text-3xl font-bold leading-tight ${colorClass}`}>
      {percentage}%
    </span>
    <div className="flex flex-col gap-1">
      <span className="text-lg font-bold text-slate-800 mt-1.5 capitalize">
        {label}
      </span>
      <span className="text-md text-slate-400 mt-0.5">
        {occupied}/{total}
      </span>
    </div>
  </div>
