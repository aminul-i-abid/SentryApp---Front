...
const AddBlockModal: React.FC<AddBlockModalProps> = ({
  ...
  useEffect(() => {
    if (blockToEdit) {
      setFormData({
        ...
        checkInTime: blockToEdit.checkInTime
          ? new Date(`2000-01-01T${blockToEdit.checkInTime}`)
          : null,
        checkOutTime: blockToEdit.checkOutTime
          ? new Date(`2000-01-01T${blockToEdit.checkOutTime}`)
          : null,
    ...
    try {
      ...
      if (response.succeeded) {
        ...
        const errorMessage =
          ...
          response.messages?.[0] ||
          (blockToEdit
            ? "Error al actualizar el pabellón"
            : "Error al crear el pabellón");
  ...
  return (
    <FormDialog
      open={open}
      onClose={handleClose}
      title={blockToEdit ? "Editar Pabellón" : "Agregar Nuevo Pabellón"}
      submitLabel={blockToEdit ? "Actualizar" : "Agregar"}
      cancelLabel="Cancelar"
      loading={loading}
      submitDisabled={!formData.name || !formData.floors}
      onSubmit={handleSubmit}
      variant="drawer"
    >
      <AddBlockForm formData={formData} onChange={handleChange} />
    </FormDialog>
  );
};
