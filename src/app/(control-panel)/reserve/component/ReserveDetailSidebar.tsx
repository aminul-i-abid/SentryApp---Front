...
import { getReserveById, deleteReservationGuest, resendMessaging, resetPinTtlock, cancelReserve, getGuestMessagingStatus, getStatusMessage } from '../reserveService';
...
import { formatDate, differenceInDays } from 'src/utils/dateHelpers';
...
const ReserveDetailSidebar: React.FC<ReserveDetailSidebarProps> = ({ open, onClose, reserveId, onOpenRoomSidebar, onGuestUpdate }) => {
    const { authState } = useAuth();
    const { enqueueSnackbar } = useSnackbar();
    const { data: user } = useUser();
    const hasTTLock = user?.modules?.ttlock === true;
    const [reserve, setReserve] = useState<ReserveDetailResponse | null>(null);
    const [companyData, setCompanyData] = useState<ContractorResponse | null>(null);
        ...
        try {
            ...
            if (response.succeeded) {
                ...
                if (response.data.companyId) {
                    try {
                        const companyResponse = await getContractorById(response.data.companyId);
    ...
    const handleWhatsAppResend = async () => {
        ...
        try {
            ...
            if (response.succeeded) {
                ...
                const updateResult = await import("../reserveService").then((mod) => mod.updateUserInfo(reserve.guests[0].rutVatId, { PhoneNumber: phoneNumber.trim() }));
                ...
                const errMsg = response.message?.length > 0 ? response.message.join(", ") : "Error al reenviar mensaje por WhatsApp";
    ...
    const handleEmailResend = async () => {
        ...
        try {
            ...
            if (response.succeeded) {
                ...
                const updateResult = await import("../reserveService").then((mod) => mod.updateUserInfo(reserve.guests[0].rutVatId, { Email: email.trim() }));
                if (!updateResult.succeeded) {
                ...
                const errMsg = response.message?.length > 0 ? response.message.join(", ") : "Error al reenviar email";
                setErrorMessage(errMsg);
                setErrorOpen(true);
            }
        } catch (error) {
    ...
    const handleResetPin = async () => {
        ...
        try {
            ...
            if (response.succeeded) {
                ...
                const errMsg = response.message && response.message.length > 0 ? response.message.join(", ") : "Error al resetear PIN";
                setErrorMessage(errMsg);
                setErrorOpen(true);
            }
        } catch (error) {
    ...
    return (
        <>
                                                    ...
                                                    <Box sx={{ flex: 1 }}>
                                                        <Box sx={{ display: 'flex', gap: 4, mb: 2, flexWrap: 'wrap' }}>
                                                            <Box>
                                                                ...
                                                                <Typography variant="body1" sx={{ fontWeight: 600, color: '#1976d2' }}>
                                                                    {companyData?.name || reserve.companyName || 'No disponible'}
                                                                </Typography>
                                                            </Box>
                                                            <Box>
                                                                <Typography variant="body2" sx={{ color: 'text.secondary', mb: 0.5 }}>
                                                        ...
                                                        <Box sx={{ display: 'flex', gap: 6, mb: 2, flexWrap: 'wrap' }}>
                                                            <Box>
                                                                ...
                                                                <Typography variant="body1" sx={{ fontWeight: 600, color: '#1976d2' }}>
                                                                    {companyData?.contactPerson || 'No disponible'}
                                                                </Typography>
                                                            </Box>
                                                            <Box>
                                                                <Typography variant="body2" sx={{ color: 'text.secondary', mb: 0.5 }}>
                                                                ...
                                                                <Typography variant="body1" sx={{ fontWeight: 600, color: '#1976d2' }}>
                                                                    {companyData?.contactPhone || companyData?.phone || 'No disponible'}
                                                                </Typography>
                                                            </Box>
                                                        </Box>
                                                        ...
                                                        <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
                                                            <Box>
                                                                ...
                                                                <Typography variant="body1" sx={{ fontWeight: 600, color: '#1976d2' }}>
                                                                    {companyData?.contactEmail || companyData?.email || 'No disponible'}
                                                                </Typography>
                                                            </Box>
                                                        </Box>
                                                    </Box>
                                                    ...
                                                    <Box sx={{ flex: 1 }}>
                                                        <Box sx={{ display: 'flex', gap: 4, mb: 4, flexWrap: 'wrap' }}>
                                                            ...
                                                            <Box>
                                                                ...
                                                                <Typography variant="body1" sx={{ fontWeight: 600, color: '#1976d2' }}>
                                                                    {calculateDuration(reserve.checkIn, reserve.checkOut)} días
                                                                </Typography>
                                                            </Box>
                                                            <Box>
                                                                <Typography variant="body2" sx={{ color: 'text.secondary', mb: 0.5 }}>
                                                                ...
                                                                <Typography variant="body1" sx={{ fontWeight: 600, color: '#1976d2' }}>
                                                                    {reserve.guests[0].shiftType === 1 ? 'Diurno' : 'Nocturno'}
                                                                </Typography>
                                                            </Box>
                                                        </Box>
                                                        <Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                                                            ...
                                                            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                                                {(() => {
                                                                    const status = getReservationStatus(reserve.status);
                                                                    return (
                                                                        <Box sx={{
                                                    ...
                                                    <Box sx={{ flex: 1, display: 'flex', gap: 3 }}>
                                                        ...
                                                        <Box sx={{ flex: 1 }}>
                                                            ...
                                                            <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
                                                                <strong>RUT/ID:</strong> {reserve.guests[0].rutVatId}
                                                            </Typography>
                                                            ...
                                                            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                                                ...
                                                                {(isSentryAdmin || isCompanyAdmin) && hasTTLock && reserve.guests[0].doorPassword && (
                                                                    <Box sx={{
                                                                        bgcolor: '#fff3e0',
                                                                        color: '#f57c00',
                                                                        px: 2,
                                                ...
                                                <Grid container spacing={2}>
                                                    ...
                                                    <Grid item xs={6}>
                                                        <Card sx={{
                                                            ...
                                                            <CardContent sx={{ p: 2, pl: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                                        <WhatsAppIcon sx={{ color: '#1976d2', fontSize: 20 }} />
                                                                ...
                                                                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.75rem', mb: 0.5 }}>
                                                                    <Box component="span" sx={{ fontWeight: 600 }}>Número:</Box> {phoneNumber || 'No especificado'}
                                                                </Typography>
                                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                    <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.75rem' }}>
                                                                        <Box component="span" sx={{ fontWeight: 600 }}>Estado:</Box> {
                                                                            loadingMessagingStatus ? 'Cargando...' :
                                                                                messagingStatus ? getStatusMessage(messagingStatus.whatsappSent, messagingStatus.whatsappMessage, 'whatsapp') :
                                                                                    phoneNumber.trim() ? 'Listo para enviar' : 'Falta número de teléfono'
                                                                        }
                                                                    </Typography>
                                                                    {messagingStatus && messagingStatus.whatsappSent === 0 && (
                                                                        <WarningIcon sx={{
                                                                            color: '#f57c00',
                                                                            fontSize: '24px',
                                                                            ml: 1
                                                    ...
                                                    <Grid item xs={6}>
                                                        <Card sx={{
                                                            ...
                                                            <CardContent sx={{ p: 2, pl: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                                                                ...
                                                                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.75rem', mb: 0.5 }}>
                                                                    <Box component="span" sx={{ fontWeight: 600 }}>Email:</Box> {email || 'No especificado'}
                                                                </Typography>
                                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                    <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.75rem' }}>
                                                                        <Box component="span" sx={{ fontWeight: 600 }}>Estado:</Box> {
                                                                            loadingMessagingStatus ? 'Cargando...' :
                                                                                messagingStatus ? getStatusMessage(messagingStatus.emailSent, messagingStatus.emailMessage, 'email') :
                                                                                    email.trim() ? 'Listo para enviar' : 'Falta correo electrónico'
                                                                        }
                                                                    </Typography>
                                                                    {messagingStatus && messagingStatus.emailSent === 0 && (
                                                                        <WarningIcon sx={{
                                                                            color: '#f57c00',
                                                                            fontSize: '24px',
                                                                            ml: 1
                                                    ...
                                                    <Grid item xs={6}>
                                                        <Card sx={{
                                                            ...
                                                            <CardContent sx={{ p: 2, pl: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                                                                ...
                                                                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.75rem' }}>
                                                                    <Box component="span" sx={{ fontWeight: 600 }}>Estado:</Box> {email.trim() ? 'Listo para resetear' : 'Falta correo electrónico'}
                                                                </Typography>
                                                            </CardContent>
                                                        </Card>
                                                    </Grid>
                                                        ...
                                                        <Grid item xs={6}>
                                                            ...
                                                            <CardContent sx={{ p: 2, pl: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                                                                    ...
                                                                    >
                                                                        {isResettingPin ? "Reseteando..." : "Resetear"}
                                                                    </Button>
                                                                </Box>
                                                                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.75rem', mb: 0.5 }}>
                                                                    <Box component="span" sx={{ fontWeight: 600 }}>Huésped:</Box> {reserve.guests[0].firstName} {reserve.guests[0].lastName}
                                                                </Typography>
                                                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                    <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.75rem' }}>
                                                                        <Box component="span" sx={{ fontWeight: 600 }}>Estado:</Box> {
                                                                            loadingMessagingStatus ? 'Cargando...' :
                                                                                messagingStatus ? getStatusMessage(messagingStatus.doorPasswordSent, undefined, 'pin') :
                                                                                    'Listo para resetear PIN'
                                                                        }
                                                                    </Typography>
                                                                    {messagingStatus && messagingStatus.doorPasswordSent === 0 && (
                                                                        <WarningIcon sx={{
                                                                            color: '#f57c00',
                                                                            fontSize: '24px',
                                                                            ml: 1
            ...
            <ConfirmationModal
                ...
                message={reserve ? `¿Estás seguro que deseas cancelar la reserva de "${reserve.guests?.[0]?.firstName} ${reserve.guests?.[0]?.lastName}"?` : ''}
                type="delete"
            />
