...
import { createBulkReservation, getBulkReserves, validateBulkReservation } from './reserveService';
...
function Reserve() {
    ...
    const [reservationStatusFilter, setReservationStatusFilter] = useState<number | undefined>(0); // Default to 'Activas' (status 0)
    ...
    const fetchReserves = async () => {
        try {
            ...
            const response = await getBulkReserves(page + 1, rowsPerPage, companyId, status);
    ...
    const handleReservationStatusFilterChange = (event: SelectChangeEvent<string>) => {
        ...
        <Root
            header={
                <div className="p-6 flex items-center justify-between">
                    ...
                    <h2 className="text-2xl font-bold">Lista de Reservas</h2>
                </div>
            }
            content={
                <div className="p-6">
                    ...
                    <TableContainer component={Paper}>
                        ...
                        >
                            ...
                            <FormControl sx={{ minWidth: 180 }} size="small">
                                <InputLabel id="reservation-status-filter-label">Estado de Reserva</InputLabel>
                                <Select
                        ...
                        <Table>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ fontSize: '0.8rem' }} align="center">Estado</TableCell>
                                    <TableCell sx={{ fontSize: '0.8rem' }} align="center">Acción</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {loading ? (
                                    ...
                                    sortedReserves
                                        .map((reserve) => (
                                            <TableRow key={reserve.id}>
                                                <TableCell sx={{ fontSize: '0.8rem' }} align="center">{`${reserve.guid}`}</TableCell>
                                                <TableCell sx={{ fontSize: '0.8rem' }} align="center">{`${reserve.guestCount}`}</TableCell>
                                                <TableCell sx={{ fontSize: '0.8rem' }} align="center">{`${reserve.companyName}`}</TableCell>
                                                <TableCell sx={{ fontSize: '0.8rem' }} align="center">{reserve.solicitante}</TableCell>
                                                <TableCell sx={{ fontSize: '0.8rem' }} align="center">
                                                    {reserve.numberOfReservations} ({reserve.activeReservations}/{reserve.cancelledReservations})
                                                </TableCell>
                                                <TableCell sx={{ fontSize: '0.8rem' }} align="center">
                                                    {reserve.created && !isNaN(new Date(reserve.created).getTime()) 
