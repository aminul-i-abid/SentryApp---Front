...
import { PaginatedMovementsResponse, CreateAdjustmentDto } from './models/NegativeAdjustment';

const endpoint = '/Movements';

export const getNegativeAdjustments = async (
    pageNumber: number = 1,
    pageSize: number = 10,
    itemId?: number,
    lotId?: number,
    ...
    params.append('transactionType', '6'); // Negative Adjustment = 6
...
export const createNegativeAdjustment = async (data: CreateAdjustmentDto): Promise<ApiResponse<number>> => {
    return genericService.create(`${endpoint}/negative-adjustment`, data);
};
