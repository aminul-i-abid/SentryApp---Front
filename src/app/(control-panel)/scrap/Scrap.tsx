...
function Scrap() {
    ...
    const { t } = useTranslation('scrap');

    // State
    const [scraps, setScraps] = useState<MovementDto[]>([]);
    ...
    const [formData, setFormData] = useState<CreateScrapDto>({
        itemId: 0,
        lotId: 0,
        warehouseId: 0,
        locationId: 0,
    ...
    const fetchDropdownData = async () => {
        try {
            ...
            if (reasonsRes.succeeded && Array.isArray(reasonsRes.data)) {
                setReasons(reasonsRes.data.filter((r: any) => r.scrap));
            }
        } catch (error) {
    ...
    const fetchScraps = async () => {
        ...
        try {
            ...
            if (response.succeeded && response.data) {
                ...
                setScraps([]);
                setTotalCount(0);
                enqueueSnackbar(response.errors?.[0] || t('scrap.messages.loadError'), { variant: "error" });
            }
        } catch (error: any) {
            console.error("Error fetching scraps:", error);
            setScraps([]);
            setTotalCount(0);
            const errorMessage = error.response?.data?.errors?.[0] || error.response?.data?.message || error.message || t('scrap.messages.loadError');
    ...
    const handleCreateScrap = async () => {
        ...
        setCreating(true);
        try {
            const response = await createScrap(formData);
            if (response.succeeded) {
                enqueueSnackbar(t('scrap.messages.createSuccess'), { variant: "success" });
                handleCloseCreateModal();
                fetchScraps();
            } else {
                enqueueSnackbar(response.message || t('scrap.messages.createError'), { variant: "error" });
            }
        } catch (error: any) {
            console.error("Error creating scrap:", error);
            const errorMessage = error.response?.data?.errors?.[0] || error.response?.data?.message || error.message || t('scrap.messages.createError');
            enqueueSnackbar(errorMessage, { variant: "error" });
        } finally {
            setCreating(false);
        }
        ...
        <Root
            ...
            header={
                <div className="p-6 flex items-center justify-between">
                    ...
                    <h2 className="text-2xl font-bold">Scrap</h2>
                </div>
            }
            content={
                <div className="p-6">
