...
const guestColumns: TableColumnDef<GuestRow>[] = [
  { id: "id", label: "ID Reserva", render: (row) => row.id },
  {
    id: "guest",
    label: "Huésped",
    render: (row) =>
  ...
  { id: "roomNumber", label: "Habitación", render: (row) => row.roomNumber },
  {
    id: "checkIn",
    label: "Check In",
    render: (row) => (row.checkIn ? formatDateForInput(row.checkIn) : "-"),
  },
  {
    id: "checkOut",
    label: "Check Out",
    render: (row) => (row.checkOut ? formatDateForInput(row.checkOut) : "-"),
  },
];

const SelectedReservationsSidebar: React.FC<
  SelectedReservationsSidebarProps
> = ({ open, onClose, rows }) => {
  ...
  const uniqueTitle = useMemo(
    () => `Editar reservas seleccionadas (${rows.length})`,
    [rows.length],
  );
  ...
  const handleSave = async () => {
    ...
    if (!globalCheckIn || !globalCheckOut) {
      enqueueSnackbar("Seleccione Check In y Check Out generales", {
        variant: "info",
    ...
    try {
      ...
      if (resp.succeeded) {
        enqueueSnackbar("Cambios validados correctamente", {
          variant: "success",
        });
        onClose();
      } else {
        const msg =
          resp.errors?.[0] || resp.message?.[0] || "Error al validar cambios";
        ...
        <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
          ...
          <Box sx={{ flex: 1, overflow: "auto", p: 2 }}>
            ...
            <StyledTable
              ...
              emptyMessage="No hay reservas seleccionadas"
              minWidth={400}
            />
          </Box>
        </Box>
