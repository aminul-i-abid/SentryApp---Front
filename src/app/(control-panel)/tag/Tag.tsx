...
import { getTags, createTag, deleteTag, getTagsPending, approveTag, rejectedTag, updateTag } from './tagService';
...
const JobTypeCard = styled(Card)<{ approved?: boolean }>(({ theme, approved }) => ({
    display: 'flex',
...
function Tag() {
    ...
    const fetchTags = async () => {
        try {
            ...
            if (response.succeeded) {
                ...
                const convertedJobs: JobType[] = (response.data as unknown as JobTitleResponse[]).map(tag => ({
                    id: tag.id,
                    title: tag.name,
        ...
        if (newTagTitle && newTagType) {
            try {
                const tagData = {
                    ...
                    tag: newTagType,
                    companyId: newTagCompany || (authState?.user?.companyId != null ? authState?.user?.companyId : null)
                };
                ...
                if (response?.succeeded) {
                    enqueueSnackbar(
                        isEditMode ? 'Cargo actualizado exitosamente' : 'Cargo creado exitosamente',
                        { variant: 'success' }
                    );
                    ...
                    const errorMessage = response?.errors?.[0] || response?.messages?.[0] || response?.message ||
                        (isEditMode ? 'Error al actualizar el cargo' : 'Error al crear el cargo');
    ...
    const uniqueCompanies = Array.from(new Set(jobTypes
                        ...
                        <Box sx={{
                            ...
                            {loading ? (
                                ...
                                filteredJobTypes.map((job) => (
                                    <JobTypeCard
                                        key={job.id}
                                        onClick={() => job.approved && (authState?.user?.role && authRoles.admin.includes(authState.user.role as string) ? handleCardClick(job) : null)}
                                    ...
                                    >
                                        ...
                                        {job.company && authState?.user?.companyId !== job.company.id && (
                                            <Typography variant="body2" color="text.secondary" sx={{ mt: 1, textAlign: 'center' }}>
                                                {job.company.name}
                                            </Typography>
                                        )}
                                ...
                                <Box sx={{ 
                                    ...
                                    <Typography>No hay tags disponibles para este rol.</Typography>
                                </Box>
                            )}
                        </Box>
                    </div>
            ...
            >
                ...
                <DialogContent sx={{ pt: 2 }}>
                    <div style={{ marginBottom: 16 }}>
                        <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Nombre del Cargo</div>
                        <TextField
                            fullWidth
                            size="small"
                            variant="outlined"
                        ...
                        />
                        <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 8, marginTop: 8 }}>Descripción</div>
                        <TextField
                            fullWidth
                            size="small"
                            variant="outlined"
                    ...
                    <div style={{ marginBottom: 16 }}>
                        <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Tipo</div>
                        ...
                        {authState?.user?.role && authRoles.admin.includes(authState.user.role as string) && (
                            <div style={{ marginTop: 16 }}>
                                <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Compañía</div>
                                <FormControl fullWidth>
                                    <Select
                                        id="company-select"
                                        value={newTagCompany}
                ...
                <DialogActions sx={{ px: 3, pb: 2, pt: 0, flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', width: '100%', mb: 2 }}>
                        ...
                        <Button
                            ...
                            color="primary"
                            sx={{ flex: 1 }}
                            disabled={!newTagTitle || !newTagType || !newTagDescription || (authState?.user?.role && authRoles.admin.includes(authState.user.role as string) && !newTagCompany)}
                        >
                            ...
                            <Table size="small">
                                <TableHead sx={{ backgroundColor: '#f5f5f5' }}>
                                    <TableRow>
                                        ...
                                        <TableCell><Typography fontWeight="bold">Compañía</Typography></TableCell>
                                ...
                                <TableBody>
                                    {pendingTags.map((tag) => (
                                        <TableRow key={tag.id} sx={{ '&:hover': { backgroundColor: '#f9f9f9' } }}>
