...
const OperationalMetricCard = ({ icon, title, value, subtitle, color, onClick = undefined }) => (
    <Paper
        elevation={0} 
        onClick={onClick}
        sx={{ 
...
const BlockOccupancyCard = ({ blockData, onBlockClick, expanded, onToggle }) => {
    ...
    return (
        ...
        >
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3, cursor: "pointer" }} onClick={onToggle}>
            ...
            <Collapse in={expanded}>
                ...
                <Grid container spacing={2}>
                    {blockData.map((block, index) => {
                        const occupancyColor = getOccupancyColor(block.occupancyPercentageToday);
                        return (
                            <Grid item xs={12} sm={6} md={4} lg={3} key={block.blockId}>
                                ...
                                >
                                    ...
                                    <Typography variant="h5" sx={{ fontWeight: 700, color: occupancyColor, my: 1 }}>
                ...
                <LinearProgress
                    ...
                    sx={{
                        ...
                        "& .MuiLinearProgress-bar": {
                            borderRadius: 6,
                            backgroundColor: occupancyData.percentage >= 80 ? "#10B981" : occupancyData.percentage >= 60 ? "#F59E0B" : "#EF4444",
                        },
                    }}
                />
                <Box
...
function DashboardTest() {
    ...
    const [isBlockOccupancySidebarOpen, setIsBlockOccupancySidebarOpen] = useState(false)
    const [selectedBlockId, setSelectedBlockId] = useState<number | null>(null)
    // Nuevos estados para colapsar las secciones
    const [isChartExpanded, setIsChartExpanded] = useState(false)
    const [isBlockOccupancyExpanded, setIsBlockOccupancyExpanded] = useState(false)
    ...
    const occupancyMainData = (() => {
        ...
        const todayOccupancy = dashboardData?.occupancySummary?.dailyOccupancy?.find((day) => new Date(day.date).toISOString().split("T")[0] === dateStr)
        ...
        const totalEnabledBeds = dashboardData.tagDetails.reduce((sum, tagDetail) => sum + (tagDetail.enabledBeds - tagDetail.lostBeds), 0)
        ...
        const percentage = totalEnabledBeds > 0 ? (occupiedBeds * 100) / totalEnabledBeds : 0
    ...
    const operationalMetrics = [
        {
            ...
            value: (() => {
                const totalDisabledRooms = dashboardData?.tagDetails?.reduce((sum, tagDetail) => sum + (tagDetail.disabledRooms || 0), 0) || 0
                return totalDisabledRooms.toString()
            })(),
            subtitle: (() => {
                const totalDisabledBeds = dashboardData?.tagDetails?.reduce((sum, tagDetail) => sum + (tagDetail.disabledBeds || 0), 0) || 0
        ...
        isAdmin && {
            ...
            value: (() => {
                const totalLostBeds = dashboardData?.tagDetails?.reduce((sum, tagDetail) => sum + (tagDetail.lostBeds || 0), 0) || 0
    ...
    const standardOccupancyData = (() => {
        if (!dashboardData?.tagDetails || !dashboardData?.occupancySummary?.capacityByTag) {
            return []
        }
        ...
        return dashboardData.tagDetails
            .map((tagDetail) => {
                ...
                const todayTagOccupancy = dashboardData.occupancySummary.occupancyByTag[tagDetail.tag.toString()]?.find((day) => new Date(day.date).toISOString().split("T")[0] === dateStr)
                ...
                // Calcular porcentaje sobre camas habilitadas
                const occupancyPercentage = enabledBeds > 0 ? (occupiedBeds * 100) / enabledBeds : 0
                ...
                return {
                    ...
                    tag: tagDetail.tag,
                    color: tagDetail.tag === 0 ? "#10B981" : tagDetail.tag === 1 ? "#F59E0B" : "#8B5CF6",
                }
            })
    ...
    return (
        <>
                    ...
                    <div className="p-6 flex items-center justify-between">
                        ...
                        {isAdmin && contractors.length > 0 && (
                            <FormControl variant="outlined" sx={{ minWidth: 300 }}>
                                ...
                                <Select labelId="contractor-select-label" id="contractor-select" value={selectedContractor} onChange={handleContractorChange} label="Contratista" size="medium">
                                    <MenuItem value="">
                        ...
                        <Grid container spacing={4}>
                            ...
                            <Grid item xs={12}>
                                <Grid container spacing={3}>
                                    ...
                                    <Grid item xs={12} md={6}>
                                        <OccupancyMainCard occupancyData={occupancyMainData} standardData={standardOccupancyData} />
                                    </Grid>
                            ...
                            <Grid item xs={12}>
                                ...
                                >
                                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3, cursor: "pointer" }} onClick={() => setIsChartExpanded(!isChartExpanded)}>
                                        <Typography variant="h6" sx={{ fontWeight: 600, color: "#1E293B" }}>
                                            ...
                                            data={{
                                                labels:
                                                    dashboardData?.occupancySummary?.dailyOccupancy
                                                        ...
                                                        ?.map((day) => new Date(day.date).toLocaleDateString("es-ES", { day: "2-digit", month: "2-digit" })) || [],
                                                datasets: [
                                                    {
                                                        label: "Total Camas",
                                                        data: dashboardData?.occupancySummary?.dailyOccupancy?.slice(0, parseInt(selectedTimeFilter))?.map((day) => day.totalBeds) || [],
                                                    ...
                                                    },
                                                    {
                                                        ...
                                                        data: dashboardData?.occupancySummary?.dailyOccupancy?.slice(0, parseInt(selectedTimeFilter))?.map((day) => day.occupiedBeds) || [],
                            ...
                            <Grid item xs={12}>
                                <BlockOccupancyCard 
                                    blockData={dashboardData?.occupancySummary?.blockOccupancy || []}
                                    onBlockClick={handleOpenBlockOccupancySidebar}
                                    expanded={isBlockOccupancyExpanded}
                                    onToggle={() => setIsBlockOccupancyExpanded(!isBlockOccupancyExpanded)}
                                />
                            </Grid>
                        </Grid>
                    </Box>
            ...
            />
            <DashboardDetailSidebar open={isSidebarOpen} onClose={handleCloseSidebar} selectedContractor={selectedContractor} />
            <LostBedsDetailSidebar open={isLostBedsSidebarOpen} onClose={handleCloseLostBedsSidebar} selectedContractor={selectedContractor} />
