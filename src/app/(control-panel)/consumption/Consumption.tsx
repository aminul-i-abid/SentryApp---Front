...
import './i18n';
import { getConsumptions, createConsumption } from "./consumptionService";
import { MovementDto, CreateConsumptionDto } from "./models/Consumption";
import { 
    getItemsWithStock,
...
function Consumption() {
    ...
    // State
    const [consumptions, setConsumptions] = useState<MovementDto[]>([]);
    ...
    const [formData, setFormData] = useState<CreateConsumptionDto>({
        itemId: 0,
        lotId: 0,
        warehouseId: 0,
        locationId: 0,
    ...
    const fetchDropdownData = async () => {
        try {
            const itemsRes = await getItemsWithStock();
    ...
    const fetchConsumptions = async () => {
        ...
        try {
            ...
            if (response.succeeded && response.data) {
                ...
                setConsumptions([]);
                setTotalCount(0);
                enqueueSnackbar(response.errors?.[0] || t('consumption.messages.loadError'), { variant: "error" });
            }
        } catch (error: any) {
            console.error("Error fetching consumptions:", error);
            setConsumptions([]);
            setTotalCount(0);
            const errorMessage = error.response?.data?.errors?.[0] || error.response?.data?.message || error.message || t('consumption.messages.loadError');
    ...
    const handleFormChange = (field: keyof CreateConsumptionDto, value: any) => {
    ...
    const handleCreateConsumption = async () => {
        if (!formData.itemId || !formData.lotId || !formData.warehouseId || 
            !formData.locationId || !formData.quantity) {
            enqueueSnackbar(t('consumption.messages.requiredFields'), { variant: "warning" });
            return;
        }

        setCreating(true);
        try {
            const response = await createConsumption(formData);
            if (response.succeeded) {
                enqueueSnackbar(t('consumption.messages.createSuccess'), { variant: "success" });
                handleCloseCreateModal();
                fetchConsumptions();
            } else {
                enqueueSnackbar(response.message || t('consumption.messages.createError'), { variant: "error" });
            }
        } catch (error: any) {
            console.error("Error creating consumption:", error);
            const errorMessage = error.response?.data?.errors?.[0] || error.response?.data?.message || error.message || t('consumption.messages.createError');
            enqueueSnackbar(errorMessage, { variant: "error" });
        } finally {
            setCreating(false);
        }
                                ...
                                {loading ? (
                                    ...
                                    consumptions.map((consumption) => (
                                        <TableRow key={consumption.id} hover>
                                            <TableCell>
                                                {new Date(consumption.movementDate).toLocaleDateString()}
                                            </TableCell>
                                            <TableCell>
                                                <Typography fontWeight={600}>{consumption.itemName}</Typography>
                                            </TableCell>
                                            <TableCell>{consumption.lotNumber}</TableCell>
                                            <TableCell>{consumption.warehouseName}</TableCell>
                                            <TableCell>{consumption.locationName}</TableCell>
