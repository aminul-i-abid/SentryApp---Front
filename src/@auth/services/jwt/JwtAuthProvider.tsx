...
import { authRefreshToken, authSignIn, authSignUp, authUpdateDbUser, authGetZendeskJwt } from '@auth/authApi';
...
function JwtAuthProvider(props: FuseAuthProviderComponentProps) {
		...
		async (credentials) => {
			...
			if (responseData.succeeded) {
				if (responseData.data.user.firstLogin == true) {
					...
					localStorage.setItem('force_reset_email', responseData.data.user.email);
	 ...
	 */
	const updateUserModules = useCallback((modules: Record<string, boolean>) => {
		if (authState.user) {
			const updatedUser = {
				...authState.user,
				modules: {
	...
	const authContextValue = useMemo(
		() =>
			({
				...
				refreshToken,
				updateUserModules
			}) as JwtAuthContextType & { updateUserModules: (modules: Record<string, boolean>) => void },
		[authState, signIn, signUp, signOut, updateUser, refreshToken, updateUserModules]
	);
